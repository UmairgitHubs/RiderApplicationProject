...
import { X, Loader2, Eye, EyeOff, Building2, MapPin, User, Mail, Lock, Warehouse, Package, Box, Phone } from 'lucide-react';
...
const hubSchema = z.object({
  ...
  createNewManager: z.boolean().default(true),
...
export default function AddHubModal({ isOpen, onClose }: AddHubModalProps) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: HubFormData) => hubsApi.create({
        ...data,
        capacity: Number(data.capacity), // Ensure number for API
        sizeSqft: Number(data.sizeSqft),
        // Only send manager fields if creating new
        managerName: data.createNewManager ? data.managerName : undefined,
        managerEmail: data.createNewManager ? data.managerEmail : undefined,
        managerPhone: data.createNewManager ? data.managerPhone : undefined,
        ...
        <Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-[95%] max-w-2xl bg-white rounded-xl shadow-xl z-50 outline-none animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-white sticky top-0 z-10 rounded-t-xl">
             <div className="flex items-center gap-3">
                 ...
                 <div>
                    <h2 className="text-lg font-bold text-gray-900">Add New Hub</h2>
                    <p className="text-sm text-gray-500">Create a new distribution center</p>
                 </div>
             </div>
             <button 
                onClick={handleClose}
            ...
            <div className="space-y-4">
                ...
                {createNewManager ? (
                    ...
                    <div className="animate-in fade-in slide-in-from-top-1 duration-200">
                        <label className="text-sm font-medium text-gray-700 mb-1 block">Select Manager</label>
                        <select 
                            {...register('managerId')}
