...
export default function VerifyOTPPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (response.success) {
        // If OTP is for login, get token and redirect
        if (response.token) {
          auth.setToken(response.token)
          router.push('/dashboard')
        } else {
          // If OTP is for password reset, redirect to reset password
          router.push(
            `/reset-password?email=${encodeURIComponent(email)}&code=${otpCode}`
          )
        }
      } else {
